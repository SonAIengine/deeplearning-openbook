...
- 도시 인구, 부의 분포, 단어 빈도 (Zipf), social network degree
- **처방**: log-log plot에서 직선이 보이면 power-law. 변환은 한계 — robust loss + capping 또는 별도 모델

### (4) Exponential

$$X \sim \text{Exp}(\lambda)$$

- 양수, monotone 감소, memoryless
- 대기 시간, decay
- **처방**: log 변환 또는 Poisson regression (count면)

### (5) Bimodal (두 봉우리)

- 두 가우시안의 mixture (또는 더 복잡)
- 두 그룹이 섞인 경우 (남/여, 정상/이상)
- **처방**: cluster를 *별도 feature*로 추가하거나, 두 그룹을 각각 모델링 (mixture model)

### (6) Zero-inflated

- 0이 매우 많고 (전체의 30%+), 0이 아닌 값은 다른 분포
- 강수량 (대부분 0), 지출액, 사기 거래
- **처방**: zero-inflated Poisson/negative binomial. 또는 *0/non-0*을 분류하고 *non-0*을 회귀하는 hurdle model

### (7) Heavy-tailed + 비대칭 (Pareto, Cauchy, t-분포)

- outlier가 자주
- 금융 수익률, 보험 청구
- **처방**: Cauchy/t-분포 likelihood, robust loss (Huber, Tukey biweight). MSE 절대 안 됨
