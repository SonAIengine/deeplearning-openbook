...
### 2.1 위험 패턴 — 즉시 display math로 분리

다음은 인라인(`$...$`)으로 쓰면 거의 확실히 깨진다. **반드시 display math(`$$ ... $$`)로 분리**.

| 패턴 | 예시 |
|---|---|
| escape brace + subscript | `$\{...\}_{...}$` |
| 한 인라인 안에 underscore 3개 이상 | `$\{(x_i, y_i)\}_{i=1}^N$` |
| 복잡한 set-builder | `$\{x \mid P(x)\}$` 류 |
...
수식을 추가/수정한 뒤 반드시 **GitHub.com에서 해당 .md 파일을 열어 KaTeX 렌더링 확인**. local preview(VSCode 등)의 마크다운 처리는 GitHub과 다를 수 있어 통과해도 GitHub에서 깨질 수 있다.
