...
## 4.1 프로젝트 구조 (hierarchy)

```
deeplearning-openbook/
├── 01_ML_Overview.md ~ 13_Task_Design_Playbook.md   # 본문 13개 챕터
├── topics/                                           # 심화 가이드 (본문에서 링크)
│   └── EDA/                                          # hub-and-spoke 구조
│       ├── README.md                                # hub — 한 페이지에 EDA 전체 그림
│       ├── 00_quick_reference.md                    # 결정 매트릭스 + 체크리스트
│       └── 01_*.md ... 15_*.md                       # 각 키워드별 sub-page (보강용)
├── assets/
│   ├── gifs/                                         # 동적 시각자료
│   └── images/                                       # 정적 시각자료 (svg, png)
├── .claude/
│   ├── settings.json                                 # PostToolUse hook 등록
│   ├── hooks/check-katex.sh                          # KaTeX 위험 패턴 자동 검사
│   └── commands/fetch-gif.md                         # /fetch-gif slash command
├── README.md
└── CLAUDE.md
```

본문에서 압축된 주제를 풀어 쓸 때 `topics/<주제>/`로 폴더를 만들고:
1. **README.md를 hub**로 — 한 페이지에 전체 그림을 self-contained 내러티브로. 핵심 키워드에 인라인 링크.
2. **각 키워드별 sub-page**를 보강 학습용으로. README만 봐도 큰 그림은 잡히고, 깊이가 필요할 때만 따라가는 hub-and-spoke 구조.

본문 해당 섹션에는 hub README로 링크. 메인 README의 "심화 주제" 표에도 등록.

### sub-page 작성 가이드 (용어 풀이)

각 sub-page에서 *처음 등장*하는 generic/익숙치 않은 용어는 인라인 한 줄 풀이 + cross-ref를 단다. 반복 풀이 X (각 sub-page는 독립적 페이지지만 같은 용어가 같은 페이지에서 반복 등장하면 처음만).

**풀이 패턴**

- 짧은 정의: `**curse of dimensionality** (차원이 커지면 학습 데이터가 지수적으로 폭발하는 현상)`
- 본문 cross-ref: `... 본문 §8 참고`
- 다른 sub-page 링크: `... [04장](04_missing_data.md)`
- bullet로 분리 (한 단락에 여러 용어 풀이가 모일 때): 새 줄 bullet으로 깔끔히

**원칙**

- 깊은 설명은 *본문 §N.M* 또는 *전용 자료*로 위임 — 인라인은 직관 잡을 정도만.
- 용어 풀이로 본문 흐름을 끊지 말 것 — 자연스럽게 괄호 안 또는 짧은 부연으로.
- 자료 톤(서술적, "교수님처럼 사고하기") 유지 — 사전식 정의가 아니라 *왜 이게 중요한지*가 함께 보이게.
