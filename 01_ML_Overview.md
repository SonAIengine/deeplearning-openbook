...
**중요한 짝 — triplet mining**: 무작위 (a, p, n)은 대부분 *easy triplet*이라 학습 신호 없음. *Semi-hard mining* (anchor-positive보다 약간 멀지만 margin 안에 있는 negative만 선택)이 표준.
...
**Adam 표준 hyperparameter** (각각 빈 줄 분리해 italic 매칭 회피)
...
$\beta_2 = 0.999$의 effective horizon은 약 1000 step. 학습이 그보다 짧으면 2차 moment 추정이 부정확해 **warmup이 사실상 필수** (§3.8).

**함정**

- LR을 그대로 두고 batch size를 키우면 학습 불안정. linear scaling rule이 Adam에서도 거의 통하지만 매우 큰 batch에선 깨진다.
- AdamW가 사실상 표준 — `Adam(weight_decay=...)` 대신 `AdamW(weight_decay=...)` 권장 (§3.7).
...
- *왜 필요한가*: 학습 초기엔 BN running 통계가 부정확하고 Adam의 2차 moment 추정이 noisy. 큰 LR로 시작하면 이 부정확한 통계 때문에 발산. (§3.6의 β2=0.999 effective horizon 1000 step과 같은 맥락.)
...
매 step 가중치가 살짝 0으로 끌려간다. $\lambda$가 hyperparameter다.

**표준 $\lambda$ (weight decay 계수)**

- *일반 신경망*: $\lambda = 10^{-4}$가 universal default. $\eta = 10^{-3}$, $\lambda = 10^{-4}$이면 매 step 가중치 norm이 약 0.0001 비율로 줄어든다.
- *Transformer/AdamW*: $\lambda = 10^{-2}$ 또는 $10^{-1}$로 훨씬 크다. AdamW가 weight decay를 정확히 처리하므로 큰 값을 넣어도 안전 (§3.7).
...
- 일반 Adam의 weight decay는 수학적으로 부정확하다 — AdamW 사용 (§3.7).
...
- *Test 시 `model.eval()`을 잊으면 dropout이 켜진 채라 결과가 매번 들쭉날쭉* (§9.1).
...
**EDA**: 가장 자주 건너뛰는 단계인데 가장 중요. 분포를 봐야 어떤 모델·전처리가 적절한지 판단 가능. 거의 모든 결정 — loss 선택, 정규화 방식, 클래스 처리, 분할 전처리 — 이 EDA에서 본 분포로 확정된다. → **상세 가이드 (책 한 권 분량): [EDA — 분포에서 결정까지](topics/EDA/README.md)**. 1부 철학·통계 기초 (Tukey, Anscombe, 분포 진단), 2부 데이터 type별 (결측·이상치·상관·표·이미지·텍스트·시계열), 3부 결정과 실전 (매트릭스·함정·도구·workflow).
