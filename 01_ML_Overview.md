...
수학적으로는 우리가 모르는 진짜 함수 $f^* : \mathcal{X} \rightarrow \mathcal{Y}$가 어딘가에 있고, 우리는 그 일부 sample $\{(x_i, y_i)\}_{i=1}^N$을 관찰한다. 우리가 만들 모델 $f_\theta$는 파라미터 $\theta$를 가지며, $f_\theta(x_i) \approx y_i$가 되도록 $\theta$를 조정한다. 학습이란 결국 **이 조정 과정**이고, 좋은 학습이란 본 적 없는 새 입력 $x_{new}$에 대해서도 $f_\theta(x_{new}) \approx f^*(x_{new})$가 되는 것 — 즉 **일반화(generalization)**가 잘 되는 학습이다.
...
셋째, **인과관계를 묻는 질문**에는 약하다. "이 약이 효과 있는가?"는 단순한 상관이 아니라 개입(intervention)의 결과를 묻는다. 관찰 데이터만으로는 confounding을 분리할 수 없고, 무작위 실험(RCT)이나 인과추론 기법이 필요하다. ML은 본질적으로 "$P(Y|X)$"의 추정 도구이지 "$P(Y|do(X))$"의 추정 도구가 아니다.
...
작은 오차엔 제곱(부드러움), 큰 오차엔 절대값(robust). 실무에서 자주 쓰는 좋은 default다. $\delta$가 hyperparameter지만 보통 1 근처로 시작한다.

**언제 무엇을 쓰는가?**
- 데이터가 깨끗하고 가우시안 가정 합리적 → MSE
...
**KL divergence와의 관계**: KL$(p || q) = \sum p \log(p/q) = \sum p \log p - \sum p \log q$. 첫 항은 $p$만의 함수니까 $q$에 대한 gradient에는 안 나타난다. 즉 KL의 $q$에 대한 gradient는 cross-entropy와 같다. 그래서 cross-entropy를 minimize하는 것 = KL divergence를 minimize하는 것. 정답 분포와 예측 분포의 거리를 줄이는 작업이다.
...
**Contrastive Loss**: SimCLR식 자기지도 학습의 기둥. 같은 sample의 두 augmentation은 가깝게, 다른 sample은 멀게. NT-Xent (normalized temperature-scaled cross entropy) loss 가 표준 form.
...
함수 $L(\theta)$를 minimize하고 싶다. 현재 위치 $\theta_0$에서 1차 Taylor 전개:

$$L(\theta_0 + \delta) \approx L(\theta_0) + \nabla L(\theta_0)^T \delta$$

작은 step $\delta$를 골라 $L$을 줄이고 싶다. $\delta = -\eta \nabla L(\theta_0)$로 놓으면:
...
이게 GD의 모든 것이다. 비밀은 없다 — **함수를 1차 근사하고 그 방향으로 살짝 움직인다**. 단순함이 그 강함이다.
...
$\beta = 0.9$가 거의 universal default. 0.99로 키우면 더 강한 관성, 0에 가까우면 순수 SGD.
...
Default: $\beta_1 = 0.9, \beta_2 = 0.999, \epsilon = 10^{-8}$. LR 1e-3이 거의 universal 시작점.
...
- **Step decay**: epoch 30, 60, 90마다 LR을 1/10. 단순하고 실증적으로 잘 동작.
- **Cosine annealing**: 코사인 곡선으로 부드럽게 감소. Transformer 표준.
- **Warmup**: 학습 초기에 0에서 본 LR로 점진적 증가. BN과 Adam의 통계가 안정될 시간을 줌.
- **OneCycle** (Smith): warmup → 큰 LR → 감소. 매우 빠른 학습.
- **Cyclic LR**: 주기적으로 LR을 키웠다 줄였다.

**왜 warmup이 큰 모델·큰 LR에서 필수인가?** 학습 초기에 BN의 running 통계가 부정확하고, Adam의 second moment 추정이 noisy하다. 큰 LR로 시작하면 이 부정확한 통계 때문에 발산할 위험. 작게 시작해서 점진 증가.
...
- **Concept drift**: $P(Y|X)$ 자체가 변함. 시간이 지나며 사용자 선호 변화.
...
매 step 가중치가 살짝 0으로 끌려간다. $\lambda$가 hyperparameter. 일반적으로 1e-4 정도.
...
표준 $p$: FC layer 0.5, conv layer 0.1~0.2 (conv는 이미 sharing이 있어서 작게).
...
**F-beta** = (1+β²)·P·R / (β²·P + R). β로 P/R 가중 조정. β=2이면 recall 더 강조.
...
**EDA**: 가장 자주 건너뛰는 단계인데 가장 중요. 분포를 봐야 어떤 모델·전처리가 적절한지 판단 가능.
