...
이 "잠깐 평면처럼 보인다"를 수학적으로 표현한 것이 **Taylor 전개**다. 매끄러운 함수라면 임의의 점 $\theta_0$ 근처에서 다음과 같이 근사된다.

$$L(\theta_0 + \delta) \approx L(\theta_0) + \nabla L(\theta_0)^T \delta$$

이건 새로운 정리가 아니라 **미분의 정의 그 자체**다. 기울기(gradient)란 *그 점 근처의 함수를 가장 잘 흉내 내는 직선·평면의 기울기*. 작은 step $\delta$ 안에서는 함수의 곡률(2차 이상 항)이 무시될 만큼 작아서, 함수가 사실상 이 평면처럼 행동한다.

여기서 GD의 모든 것이 따라온다 — *현재 점에서 1차 평면을 그리고, 그 평면 위에서 가장 빨리 내려가는 방향으로 한 step 가고, 새 위치에서 다시 평면을 그리고, 또 한 step. 반복.*
