...
**EDA**: 가장 자주 건너뛰는 단계인데 가장 중요. 분포를 봐야 어떤 모델·전처리가 적절한지 판단 가능. 거의 모든 결정 — loss 선택, 정규화 방식, 클래스 처리, 분할 전략 — 이 EDA에서 본 분포로 확정된다. → **상세 가이드: [EDA → 모델·전처리 결정 흐름](topics/EDA_guide.md)** (점검 항목, 결정 매트릭스, 데이터 type별 체크리스트, 흔한 함정).
